...
**분석 기준을 세운 과정**
- 처음에는 모든 클랜을 같은 기준으로 모델링하려고 했지만, 전체 데이터의 `90.5%`가 비활성에 가까운 유령 클랜이라는 점을 확인했습니다. 이 상태에서 모델을 만들면 "대부분의 클랜은 죽어 있다"는 결론만 강해지고, 실제 클랜장이 참고할 수 있는 액션이 나오기 어렵다고 판단했습니다.
- 그래서 유령 클랜을 단순 삭제하지 않고 먼저 분류 기준으로 정의했습니다. `num_members < 5`, `war_total == 0`, `clan_capital_points == 0`처럼 활동성이 거의 없는 조건을 기준으로 비활성 클랜을 구분하고, 모델링과 핵심 EDA는 활성 클랜 중심으로 진행했습니다.
- 승률도 그대로 믿지 않았습니다. 100% 승률 클랜이 과도하게 많았고, 그중 일부는 전쟁을 1~3판만 치른 표본이었습니다. 그래서 성과 해석에는 `war_total >= 20` 기준을 적용해 허수 성과를 줄였습니다.
- 모델 AUC가 비정상적으로 높게 나왔을 때는 좋은 결과로 받아들이지 않고 데이터 누수를 의심했습니다. 클랜 생존 여부와 직접적으로 맞물리는 변수를 제거하고, 실제 예측 시점에 사용할 수 있는 변수 중심으로 다시 구성했습니다.
...
이 프로젝트의 목표는 "정확도 높은 모델을 만들었다"에서 끝나는 것이 아니라, 클랜장이 어떤 운영 판단을 할 수 있는지까지 연결하는 것이었습니다.
...
- 리그 예측 모델은 정확히 같은 티어를 맞히는 것보다 `±1 티어` 범위에서 현실적 목표 구간을 제시하는 데 의미가 있었습니다. 따라서 "이 클랜은 다음 시즌에 Champion을 노린다"보다 "현재 조건에서는 Gold~Crystal 구간을 안정 목표로 잡고, 부족한 지표를 보완한다"는 식의 가이드가 더 현실적이라고 판단했습니다.
...
- 판단 기준: 성능이 높아도 실제 예측 시점에 알 수 없는 변수가 섞이면 사용할 수 없다고 보고, 설명 가능한 운영 지표 중심으로 재학습
...
- 판단 기준: 단순 정확도만 높이는 모델보다, 소수 리그를 완전히 놓치지 않는 모델과 현실적 목표 티어를 제시하는 모델을 우선 검토
- 성능
