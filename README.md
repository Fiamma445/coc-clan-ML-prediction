...
- 검토 과정: 성능이 높아도 실제 예측 시점에 알 수 없는 변수가 섞이면 사용할 수 없다고 보고, 설명 가능한 운영 지표 중심으로 다시 학습
...
- 검토 과정: 단순 정확도만 높이는 모델보다, 소수 리그를 완전히 놓치지 않고 현실적인 목표 티어를 제시할 수 있는 모델을 우선 검토
- 성능
...
- 결과 해석
  - 모델 성능은 최종 결론이 아니라 운영 의사결정의 우선순위를 정하기 위한 도구로 해석
  - `11명 이상 활동 인원`, `전쟁 20판 이상 성과`, `±1 티어 목표`처럼 실제 행동으로 바꿀 수 있는 기준을 함께 제시
- 한계
  - 텍스트/채팅 행동 데이터 부재
